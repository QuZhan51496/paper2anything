#!/usr/bin/env bash
# tools/install-linux.sh — 把 paper2anything 的 5 个 skill 注册到 Claude Code（Linux 版）
#
# 做的事：
...
#   --shell-init   把 .env 自动导出写进 shell 启动文件（默认 ~/.bashrc；登录 shell 是
#                  zsh 时写 ~/.zshrc），新开 shell 即加载凭据（幂等；不加只打印建议）。
#
# 不做的事（需 sudo / 系统级，按提示手动装）：
#   - 不装系统包（sudo apt install poppler-utils / libreoffice / nodejs）
#   - 不装 npm 全局包（pptxgenjs）
#
# macOS 用户请改用 tools/install-macos.sh（用 Homebrew、缓存/路径不同）。
...
# Linux 默认 bash→~/.bashrc；登录 shell 是 zsh 时写 ~/.zshrc。
detect_shell_rc() {
  case "${SHELL:-}" in
    *zsh) echo "$HOME/.zshrc" ;;
    *)    echo "$HOME/.bashrc" ;;
...
# pptxgenjs + react-icons/react/react-dom/sharp（npm 全局，paper2slides）
if command -v npm >/dev/null 2>&1; then
  ...
  if [[ -z "$npm_missing" ]]; then
    ...
    yellow "  [missing] npm 全局包:$npm_missing — 跑: sudo npm install -g pptxgenjs react-icons react react-dom sharp"
  fi
fi
