...
注：spec 里**不需要**写 `sizing` 字段——render 端会忽略它（因为已经在 Python 端把 (w, h) 调成实际占位，不再依赖 PptxGenJS 的 sizing）。写了也无害，会被自动 pop 掉。
...
不再有"每页 ≤ N 词 / 每 bullet ≤ N 词"这类硬限。文字量服务于一个目标：
**这页被内容填满、既不留白也不超框**。
...
> 5. 对每个"引导符 icon + 配对 bullet text"：`icon.y + icon.h/2` 与
>    `text.y + text.h/2` 差是多少？是否 ≤ 0.02"？超过即引导符与文字没垂直
>    对齐（符号肉眼可见偏上/偏下）——**hard issue**（见"视觉丰富度建议 A"
